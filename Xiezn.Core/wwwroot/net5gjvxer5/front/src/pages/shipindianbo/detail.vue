...
<div>
	<div :style='{"padding":"16px 12%","margin":"0px auto","borderColor":"#d8d8d8","borderRadius":"0px","background":"none","borderWidth":"0 0 3px","width":"100%","borderStyle":"solid"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'/'" :style='{"fontSize":"16px","lineHeight":"1","justifyContent":"flex-start","display":"flex"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/shipindianbo"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"padding":"10px 0 0","margin":"0","flexWrap":"wrap","background":"none","display":"flex","width":"60%","position":"relative","justifyContent":"space-between","order":"2"}'>
			...
			<div class="info" :style='{"width":"100%","padding":"0px","margin":"0","background":"none","flex":"1","order":"2"}'>
				...
				<div class="item" :style='{"padding":"0px 10px","margin":"0","borderColor":"#28890b10","background":"#fff","borderWidth":"0px","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"8px 10px 0","color":"#000","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"24px","height":"auto"}'>视频分类</div>
					<div  :style='{"padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#666","flex":"1","height":"auto"}'>{{detail.shipinfenlei}}</div>
				</div>
				<div class="item" :style='{"padding":"0px 10px","margin":"0","borderColor":"#28890b10","background":"#fff","borderWidth":"0px","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"8px 10px 0","color":"#000","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"24px","height":"auto"}'>视频标签</div>
					<div  :style='{"padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#666","flex":"1","height":"auto"}'>{{detail.shipinbiaoqian}}</div>
				</div>
				<div class="item" :style='{"padding":"0px 10px","margin":"0","borderColor":"#28890b10","background":"#fff","borderWidth":"0px","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
				...
				<div class="item" :style='{"padding":"0px 10px","margin":"0","borderColor":"#28890b10","background":"#fff","borderWidth":"0px","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"8px 10px 0","color":"#000","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"24px","height":"auto"}'>浏览时长</div>
					<div  :style='{"padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#666","flex":"1","height":"auto"}'>{{detail.browseduration}}</div>
				</div>
				<div class="btn" :style='{"width":"100%","padding":"20px 112px 0","borderRadius":"0","flexWrap":"wrap","background":"#fff","display":"block"}'>
					<el-button :style='{"border":"1px solid #3795c4","cursor":"pointer","padding":"0 20px","margin":"0 10px 20px 0","outline":"none","color":"#3795c4","borderRadius":"6px","background":"none","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('shipindianbo','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 10px 10px 0","outline":"none","color":"#fff","borderRadius":"6px","background":"#3795c4","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('shipindianbo','删除')" @click="delClick">删除</el-button>
					<!-- hasChat 无 -->
				</div>
			</div>
		</div>
			...
			<!-- 热门信息 -->
			<div class="hot" :style='{"padding":"20px 20px 0","margin":"20px 0 0","textAlign":"center","background":"#fff","flex":"1","width":"60%","height":"auto","order":"5"}'>
			  <div :style='{"padding":"0 0 0 10px","borderColor":"#3795c4","color":"#3795c4","textAlign":"left","display":"inline-block","background":"none","borderWidth":"0 0 0 6px","width":"100%","lineHeight":"1","fontSize":"32px","borderStyle":"solid","fontWeight":"600","height":"32px"}'>推荐信息</div>
			  <div :style='{"padding":"0 0 5px 0","margin":"30px 0 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			    <div v-for="item in hotList" :key="item.id" :style='{"cursor":"pointer","width":"23%","padding":"0","margin":"0 0 20px","background":"#fff","height":"auto"}' @click="toDetail(item)">
			      <img :style='{"width":"100%","objectFit":"cover","display":"block","height":"150px"}' v-if="item.shipinfengmian && item.shipinfengmian.substr(0,4)=='http'" :src="item.shipinfengmian" alt="">
			      <img :style='{"width":"100%","objectFit":"cover","display":"block","height":"150px"}' v-else :src="baseUrl + (item.shipinfengmian?item.shipinfengmian.split(',')[0]:'')" alt="">
			      <div :style='{"padding":"0px 10px","margin":"0 0 6px","borderColor":"#ddd","whiteSpace":"nowrap","color":"#333","textAlign":"left","overflow":"hidden","background":"none","borderWidth":"0 0 0px","width":"100%","lineHeight":"40px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"dashed","height":"40px"}'>{{item.shipinmingcheng}}</div>
			      <!-- <div :style='{"padding":"0 10px","color":"#999","textAlign":"left","display":"block","width":"100%","lineHeight":"24px","fontSize":"14px"}'>2022-02-02</div> -->
			    </div>
			  </div>
			</div>
		
		<video :style='{"border":"0","margin":"0px 2% 0 0","outline":"none","borderRadius":"4px","display":"block","width":"38%","order":"4","height":"450px"}' :src="baseUrl +  detail.shipinneirong" controls="controls">
			您的浏览器不支持视频播放
		</video>
  ...
  export default {
    ...
    methods: {
        ...
        updateBrowseDuration() {
            this.$http.post('shipindianbo/updateBrowseDuration/'+this.detail.id+"?duration=3", {}).then(res => {
            });
        },
        toDetail(item) {
			let params = {
          ...
          this.$router.push({path: '/index/shipindianboDetail', query: params});
          this.init();
        ...
        getHotList() {
          let autoSortUrl = "";
          autoSortUrl = "shipindianbo/autoSort";
      ...
      storeup(type) {
        if (type == 1 && !this.isStoreup) {
          ...
          this.$http.post('storeup/add', this.storeupParams).then(res => {
            if (res.data.code == 0) {
			  ...
			  this.$http.post('shipindianbo/update', this.detail).then(res => {});
              this.$message({
                type: 'success',
                message: '收藏成功!',
                duration: 1500,
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'shipindianbo', userid: localStorage.getItem('frontUserid')}}).then(res => {
    ...
    getThumbsupOrCrazilyStatus() {
        if(localStorage.getItem("frontToken")) {
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 21, refid: this.detail.id, tablename: 'shipindianbo', userid: localStorage.getItem('frontUserid')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
            ...
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 22, refid: this.detail.id, tablename: 'shipindianbo', userid: localStorage.getItem('frontUserid')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
      ...
      getDiscussList(page) {
        this.$http.get('discussshipindianbo/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('discussshipindianbo/delete',[id]).then(res=>{
				  if(res.data&&res.data.code==0){
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussshipindianbo/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
				this.form.score = 0
	  ...
	  addDiscussNum(type){
		...
		// 修改
		editClick(){
			this.$router.push(`/index/shipindianboAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此视频点播？')
			  .then(_ => {
			    this.$http.post('shipindianbo/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
