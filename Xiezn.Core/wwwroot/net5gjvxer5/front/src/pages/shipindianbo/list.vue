...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 0","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","display":"flex","width":"100%","borderStyle":"solid","height":"auto","order":"2"}'>
      <el-form-item :style='{"margin":"0 5px 0px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>视频名称：</div>
	  ...
	  <el-button v-if="btnAuth('shipindianbo','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 15px","margin":"0px 0px 0 0","color":"#fff","display":"inline-block","outline":"none","borderRadius":"30px","background":"#1c6a6d","width":"90px","fontSize":"14px","lineHeight":"40px","height":"40px"}' type="primary" @click="add('/index/shipindianboAdd')"><i v-if="false" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"minHeight":"699px","padding":"20px 0","margin":"20px 0 0","borderRadius":"0px","background":"none","flex":"1","width":"calc(100% - 310px)","float":"left","order":"5"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0 0 0 2%","color":"#999","flexWrap":"wrap","background":"none","display":"flex","width":"100%","fontSize":"14px","height":"auto"}'>
			<div :style='{"border":"0px solid #ddd","cursor":"pointer","padding":"0 0 0px","boxShadow":"1px 2px 9px #ccc","margin":"0 2% 30px 0","display":"flex","justifyContent":"flex-end","overflow":"hidden","flexWrap":"wrap","background":"#fff","width":"23%","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<img @click.stop="imgPreView(item.shipinfengmian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"240px"}' v-if="item.shipinfengmian && item.shipinfengmian.substr(0,4)=='http'" :src="item.shipinfengmian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.shipinfengmian?item.shipinfengmian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"240px"}' v-else :src="baseUrl + (item.shipinfengmian?item.shipinfengmian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"0 10px","borderColor":"#ddd","margin":"0 0 6px","whiteSpace":"nowrap","color":"#333","textAlign":"left","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"32px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"dashed"}' class="name ">{{item.shipinmingcheng}}</div>
	...
	<div class="hot" :style='{"padding":"0px","margin":"40px 0 0 20px","borderRadius":"0px","background":"#f6f6f6","width":"250px","height":"auto","order":"6"}'>
	  ...
	  <div :style='{"padding":"10px 10px 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
	    <div v-for="item in hotList" :key="item.id" :style='{"cursor":"pointer","width":"100%","margin":"0 0 10px","background":"#fff","height":"auto"}' @click="toDetail(item)">
	      <img :style='{"width":"100%","objectFit":"cover","float":"left","display":"block","height":"180px"}' v-if="item.shipinfengmian && item.shipinfengmian.substr(0,4)=='http'" :src="item.shipinfengmian.split(',')[0]" alt="">
	      <img :style='{"width":"100%","objectFit":"cover","float":"left","display":"block","height":"180px"}' v-else :src="baseUrl + (item.shipinfengmian?item.shipinfengmian.split(',')[0]:'')" alt="">
	      <div :style='{"padding":"0 10px","borderColor":"#eee","whiteSpace":"nowrap","color":"#333","textAlign":"left","float":"right","overflow":"hidden","borderWidth":"0px","width":"100%","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}'>{{item.shipinmingcheng}}</div>
    ...
    created() {
		...
		this.$http.get('option/shipinfenlei/shipinfenlei').then(res => {
			if (res.data.code == 0) {
				...
				this.selectOptionsList.push({name:'视频分类',list:this.shipinfenleiOptions,tableName: 'shipinfenlei',check: -1})
			}
		});
		this.$http.get('option/shipinbiaoqian/shipinbiaoqian').then(res => {
			if (res.data.code == 0) {
				this.shipinbiaoqianOptions = res.data.data;
				this.selectOptionsList.push({name:'视频标签',list:this.shipinbiaoqianOptions,tableName: 'shipinbiaoqian',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.indexQueryCondition) {
          searchWhere.shipinmingcheng = '%' + this.indexQueryCondition + '%';
        ...
        if (this.formSearch.shipinbiaoqian != '') searchWhere.shipinbiaoqian = this.formSearch.shipinbiaoqian;
        ...
        this.$http.get(`shipindianbo/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
