...
<div class="content" :style='{"padding":"0px 30px 30px","margin":"0 auto","alignItems":"flex-start","borderRadius":"0 0 10px 10px","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230830/d58ecc944fb54dbfa18ecd2b64e92170.png) no-repeat center top / cover","display":"flex","width":"calc(100% - 60px)","fontSize":"16px"}'>
	...
	<!-- echarts -->
	<!-- 2 -->
	<div class="type2" :style='{"padding":"0","margin":"0","display":"flex","justifyContent":"space-between","alignContent":"flex-start","borderRadius":"8px","flexWrap":"wrap","background":"none","flex":"1","width":"100%","position":"relative","height":"460px","order":"10"}'>
		<div id="shipindianboChart1" class="echarts1" v-if="isAuth('shipindianbo','首页统计')"></div>
		<div id="shipindianboChart2" class="echarts2" v-if="isAuth('shipindianbo','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getyonghuCount();
		this.getshipindianboCount();
		this.shipindianboChat1();
		this.shipindianboChat2();
	},
	methods:{
		// 词云
		...
		getshipindianboCount() {
			this.$http({
				url: `shipindianbo/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.shipindianboCount = data.data
				}
			})
		},
		shipindianboChat1() {
        ...
        var shipindianboChart1 = echarts.init(document.getElementById("shipindianboChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                shipindianboChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    shipindianboChart1.resize();
                };
            }
        });
      })
    ...
    shipindianboChat2() {
      this.$nextTick(()=>{
        ...
        var shipindianboChart2 = echarts.init(document.getElementById("shipindianboChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                shipindianboChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    shipindianboChart2.resize();
                };
            }
        });
      })
